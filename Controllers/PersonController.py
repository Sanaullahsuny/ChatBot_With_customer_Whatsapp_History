...
class PersonController:
    ...
    def get_person():
        try:
            ...
            people = Person.query.filter_by(isDeleted=False,type=False).all()
    ...
    @staticmethod
    def check_user(number, name):

        if ':' in number:
            _, number_part = number.split(':', 1)
        else:
            number_part = number

        if number_part.startswith('+92'):

            local_number = '0' + number_part[3:]
        elif number_part.startswith('92'):

            local_number = '0' + number_part[2:]
        else:
            local_number = number_part

        # Check if user exists in the database
        person = Person.query.filter_by(Phno=local_number, isDeleted=False).first()
        if person:
            return person.id
        else:
